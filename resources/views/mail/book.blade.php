...
<table cellpadding="0" cellspacing="0" width="100%"
    ...
    <tr>
        <td style="padding: 20px; background-color: #f8f8f8;">
            <table width="100%" style="font-size: 14px;">
                ...
                <tr>
                    <td style="padding-top: 15px;"><strong>Check-in</strong><br>{{ $arrivalDate }}
                        , {{ $hotel->checkin ?? ''}}</td>
                    <td style="padding-top: 15px;" align="right"><strong>Check-out</strong><br>{{ $departureDate }}
                        , {{ $hotel->checkout ?? ''}}</td>
                </tr>
            </table>
        </td>
    ...
    <tr>
        <td style="padding: 15px 20px;">
            ...
            <p style="margin: 5px 0 0; font-size: 13px; color: #666;">Room {{ $book->room_count ?? ''}}.
                For {{ $book->adult + $book->child ?? ''}} quests</p>
        </td>
    </tr>
            ...
            <table width="100%" style="font-size: 14px;" cellpadding="6">
                ...
                @isset($rate)
                    <tr>
                        ...
                        <td align="right">{{ $rate->title_en ?? $rate->title ?? $rate_id}}</td>
                    </tr>
                    <tr>
                ...
                <tr>
                    ...
                    <td align="right" style="color: #000; font-size: 16px;">
                        <strong>{{ $book->sum }} {{ $book->currency}}</strong></td>
                </tr>
            </table>

            <div style="text-align: center; margin-top: 25px;">
                <a href="{{ route('index') }}/auth/userbooks/show/{{$book->id}}"
                   style="display: inline-block; background-color: #0061ae; color: #fff; text-decoration: none; padding: 12px 24px; border-radius: 4px; font-weight: bold;">View booking</a>
            </div>
        </td>
    </tr>
</table>
