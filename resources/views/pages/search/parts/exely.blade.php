@php
    ...
    /** @var array $item */
    $room   = $item['roomStay'] ?? null;   // stdClass из Exely
    //dd($room);
    $hotel  = \App\Models\Hotel::where('exely_id', $room->propertyId)->first();

    // --- Exely property ---
    $property   = $room->property ?? null;

    \Log::debug('EXELY PROPERTY SAMPLE', (array) $property);

    // Название / адрес / рейтинг: сначала Exely, потом локальный Hotel
    $exTitle = $property->name ?? null;

    $exAddress  = $property->address ?? null;
    $exRating   = $property->rating  ?? null;

    if (is_array($hotel)) {
        $hotelId   = $hotel['id']    ?? null;
        $locTitle = $hotel?->title ?? '';
        $locRating = $hotel['rating'] ?? null;
        $locAddr   = $hotel['address'] ?? null;
    } else {
        $hotelId   = $hotel?->id;
        $locTitle  = $hotel?->title ?? '';
        $locRating = $hotel?->rating ?? null;
    ...
    $title = $exTitle ?: $locTitle;
    $address = $exAddress ?: $locAddr;
    $rating  = is_numeric($exRating) ? $exRating : $locRating;

    // --- Картинка: сначала Exely, потом локальная ---
    $exImage = null;
    if ($property && isset($property->mainImageUrl)) {
        $exImage = $property->mainImageUrl;
    } elseif ($property && isset($property->photos[0]->url)) {
        $exImage = $property->photos[0]->url;
    }


    $images = (!$exImage && $hotelId)
        ? Image::where('hotel_id', $hotelId)->get()
        : collect();

    $amenities = ($hotel && $hotel->amenity)
        ? explode(',', $hotel->amenity->services)
        : [];
    ...
    // data-id: либо локальный hotel.id, либо Exely propertyId
    $dataId = $hotelId;
    if (!$dataId && $room && isset($room->propertyId)) {
        $dataId = $room->propertyId;
    }
@endphp

<div class="search-item ex" data-id="{{ $dataId }}">
    <div class="row">
                ...
                <div class="owl-carousel owl-slider">
                    <div class="slider-item">
                        ...
                        @elseif($images->isNotEmpty())
                            <img src="{{ Storage::url($images->first()->image) }}" alt="{{ $title }}">
                            @foreach($images->slice(1, 2) as $file)
                                <div class="primary">
                                    <img src="{{ Storage::url($file->image) }}" alt="{{ $title }}">
                                </div>
                            @endforeach
                        @else
                            <img src="{{ route('index')}}/img/noimage.png" alt="{{ $title }}">
                        @endif
                    </div>
                </div>
                <div class="text-wrap">
                    <div class="price">
                        @lang('main.from')
                        {{ number_format((float)($item['conv_total'] ?? 0), 0, '.', ' ') }}
                        {{ $item['conv_symbol'] ?? '' }}
                    </div>
                    <div class="night">@lang('main.night')</div>
                    @if($rating)
                        <div class="rating">
                            <img src="{{ route('index') }}/img/star.svg" alt=""> {{ $rating }}
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class="col-md-6 order-2">
            <div class="wrap">
                ...
                <div class="btn-wrap">
                    <form method="GET" action="{{ route('findHotelExely', $room->propertyId) }}">
