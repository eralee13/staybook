            ...
            <div class="row">
                <div class="col-lg-6 col-md-12">
                    <h1 data-aos="fade-up" data-aos-duration="2000">@lang('main.booking_cancellation')</h1>
                        ...
                        @else
                            @lang('main.non_refundable'): {{ round($book->cancel_penalty) }} {{ $book->currency ?? 'RUB' }}
                        @endif
                    <form action="{{ route('cancel_confirm_hs') }}">
                        <div class="form-group">
                            <label for="">@lang('main.booking_number')</label>
                            <input type="text" readonly value="{{ $request->number }}" name="number">
                        </div>
                        <input type="hidden" name="cancelPrice" value="{{ $book->cancel_penalty }}">
